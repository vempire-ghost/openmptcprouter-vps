...
VPSPATH="vempire-ghost/openmptcprouter-vps/refs/heads/develop"
VPS_PUBLIC_IP=${VPS_PUBLIC_IP:-$(wget -4 -qO- -T 2 http://ip.openmptcprouter.com)}
VPSURL="https://raw.githubusercontent.com/"
REPO="168.75.85.65:8000"
CHINA=${CHINA:-no}
...
#	wget http://${REPO}/openmptcprouter.gpg.key -O /etc/apt/trusted.gpg.d/openmptcprouter.gpg
#}
...
#	fi
	...
	echo "deb [arch=amd64] http://${REPO} buster main" > /etc/apt/sources.list.d/openmptcprouter.list
	...
	if [ -n "$(echo $OMR_VERSION | grep test)" ]; then
		echo "deb [arch=amd64] http://${REPO} next main" > /etc/apt/sources.list.d/openmptcprouter-test.list
